use std::str::FromStr;
use serenity::all::{ChannelId, CommandInteraction, CommandOptionType, Context, CreateCommand, CreateCommandOption, MessageBuilder};
use crate::model::moderator::{Moderator, SuggestedRating, SuggestedScore};
use crate::model::request_score::LevelLength;
...
use crate::service::moderator_service::ModeratorService;
use crate::util::discord::invoke_ephermal;
...
pub async fn run_send_level(ctx: &Context, command: &CommandInteraction) {
    let level_id = command
        ...
        .unwrap()
        .unsigned_abs();
    let suggested_score = SuggestedScore::from_str(command
        .data
        .options
        .get(1)
        .unwrap()
    ...
    let suggested_rating = SuggestedRating::from_str(command
        .data
        .options
        .get(2)
        .unwrap()
    ...
    match service.send_level(&ctx, &command, send_level_request).await {
        Ok(level_request_data) => {
            ...
            send_level_message.push(format!("\"{}\" ({})", level_request_data.level_name, level_request_data.level_id));
            if level_request_data.level_length == LevelLength::Platformer {
                if send_level_request.suggested_score == SuggestedScore::NoRate {
                    ...
                    send_level_message.push_bold(format!(
                        "{}, {} Moons!",
                        serde_json::to_string(&suggested_rating).unwrap(),
                        serde_json::to_string(&suggested_score).unwrap()
                    ));
                ...
                if send_level_request.suggested_score == SuggestedScore::NoRate {
                    ...
                    send_level_message.push_bold(format!(
                        "{}, {} Stars!",
                        serde_json::to_string(&suggested_rating).unwrap().replace("\"", ""),
                        serde_json::to_string(&suggested_score).unwrap().replace("\"", "")
                    ));
                }
            }
