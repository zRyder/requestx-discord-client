...
use crate::{
	...
	model::error::{discord_user_error::DiscordUserError, format_cooldown},
	...
	match service.get_discord_user(discord_user_id).await {
		Ok(discord_user) => {
			if let Some(last_request_time) = discord_user.last_request_time {
				if let Some(duration_string) =
					format_cooldown(last_request_time, discord_user.request_cooldown as i64)
				{
					invoke_ephemeral(
						format!(
		...
		Err(get_discord_user_error) => {
			...
			invoke_ephemeral(&get_discord_user_error.to_string(), &ctx, &command).await;
		}
	}
}
	...
	match user_service.get_discord_user(discord_user_request_id).await {
		...
		Err(get_discord_user_error) => {
			if matches!(get_discord_user_error, DiscordUserError::UserDoesNotExist) {
				return invoke_ephemeral("You can request a level now", &ctx, &command).await;
			}
