...
impl<'a> RequestXApiClient<'a> {
	...
	pub async fn get_user(&self, discord_id: u64) -> Result<Option<DiscordUser>, DiscordUserError> {
		...
		if status_code.eq(&StatusCode::NOT_FOUND) {
			...
			let discord_user_data: DiscordUser =
	...
	fn handle_level_request_error(
		...
		match response_status {
			...
			StatusCode::TOO_MANY_REQUESTS => LevelRequestError::UserOnCooldown(
				serde_json::from_str::<UserOnCooldownError>(&*response_body).unwrap_or_default()
			),
