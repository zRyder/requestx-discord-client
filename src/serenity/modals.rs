...
use serenity::all::{Component, CreateComponent, CreateLabel, CreateSelectMenu, CreateSelectMenuKind, CreateSelectMenuOption, LabelComponent, ModalInteraction};
...
pub fn get_request_level_modal<'a>() -> CreateModal<'a> {
	...
	rows.push(CreateComponent::Label(
		CreateLabel::select_menu(
			...
			.description("Select \"Yes\" if you would like to potentially receive feedback on this \
			level request.")
	));
	rows.push(CreateComponent::Label(
		CreateLabel::select_menu(
			...
			.description("Select \"Yes\" if you would like to be pinged when your \
...
fn get_request_level_request_rating_options<'a>() -> Vec<CreateSelectMenuOption<'a>> {
	vec![
		...
		CreateSelectMenuOption::new(
			"Harder, 6 Stars/Moons",
			"Six"
		),
		CreateSelectMenuOption::new(
			"Harder, 7 Stars/Moons",
			"Seven"
		),
		CreateSelectMenuOption::new(
			"Insane, 8 Stars/Moons",
			"Eight"
		),
		CreateSelectMenuOption::new(
			"Insane, 9 Stars/Moons",
			"Nine"
		),
		CreateSelectMenuOption::new(
			"Demon, 10 Stars/Moons",
			"Ten"
		)
	]
}
				...
				match &label.component {
					LabelComponent::SelectMenu(select_menu) => {
						if ids_to_search.contains(&select_menu.custom_id.to_string()) {
							components_map.insert(select_menu.custom_id.to_string(), select_menu.values.get(0).unwrap().to_string());
						}
					}
					LabelComponent::InputText(input_text) => {
						if let Some(input_text_value) = &input_text.value {
							if ids_to_search.contains(&input_text.custom_id.to_string()) {
								components_map.insert(input_text.custom_id.to_string(), input_text_value.to_string());
							}
						}
					}
					_=> {}
