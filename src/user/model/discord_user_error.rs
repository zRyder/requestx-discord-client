	...
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		match self {
			GDAccountLinkError::GDAccountDoesNotExist => {
				write!(f, "GD Account or user does not exist")
			}
			...
			GDAccountLinkError::DiscordAccountAlreadyLinked => {
				write!(f, "User has already been linked to a GD account")
			}
			...
			GDAccountLinkError::RequestXApiError(_) => {
				write!(f, "The server returned an unknown error")
			}
		}
	}
}
