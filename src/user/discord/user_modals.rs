...
use crate::serenity::discord::{invoke_modal_ephemeral, log_action_to_discord, log_error_to_discord};
...
pub async fn run_init_gd_account_link_modal(
    ...
    let modal_inputs = extract_modal_components(
        &modal_interaction,
        vec!["gd-username".to_string()]
    );
    let discord_user_id = modal_interaction.user.id.get();
    let Some(gd_username_input) = modal_inputs
        .get(&"gd-username".to_string())
    else {
    ...
    match user_service.init_gd_account_link(user_gd_account_link_request).await {
        ...
        Err(init_gd_account_link_error) => {
            error!("Unable to init gd account link: {}", init_gd_account_link_error);
